...
const createFormRequest = (age = '10') => {
...
describe('parseForm', () => {
  ...
  test('parses objects keys of FormData from FormData using a schema', async () => {
    ...
    const image = new File([''], 'image.jpeg', {
      type: 'image/jpeg',
...
describe('parseFormSafe', () => {
  ...
  test('parses FormData from FormData using a ZodEffects schema', async () => {
    ...
    if (!zodixResult.success && !zodResult.success) {
      expect(zodixResult.error?.issues).toStrictEqual(zodResult.error?.issues)
    }
...
// Custom FormData parser that casts a set of key to File
  ...
  return (formData: FormData) => {
    ...
    for (const [key, value] of formData) {
      ...
      if (fileKeys.includes(key) && value instanceof File) {
        ...
        values[key] = [currentVal, value.toString()]
      } else {
        values[key] = value
      }
    }
